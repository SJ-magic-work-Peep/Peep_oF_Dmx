...
void GUI_GLOBAL::setup(string GuiName, string FileName, float x, float y)
{
	...
	********************/
	Group_RotStage.setup("RotState");
		Group_RotStage.add(RotStage_threshToDetectBlack.setup("th:DetectBlack", 500, 10, 1000));
	...
	Group_Table.setup("Table");
		Group_Table.add(Table_dAngle_Down.setup("dAngle_Down", 20, 0, 90));
		Group_Table.add(Table_dAngle_Up.setup("dAngle_Up", 20, 0, 90));
	...
	Group_DMX_T.setup("DMX T");
		Group_DMX_T.add(DMX_T__Figure8.setup("T:Figure8", 2200, 500, 30000));
		Group_DMX_T.add(DMX_T__Coloring.setup("T:Coloring", 2388, 100, 4000));
	gui.add(&Group_DMX_T);
