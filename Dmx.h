...
class PAN_TILT{
public:
	PAN_TILT() : Pan(0), Tilt(0) { }
	PAN_TILT(int _Pan, int _Tilt) : Pan(_Pan), Tilt(_Tilt) { }
...
private:
	...
	int limit(int val){
		if(0 < val)				return 0;
		else if(VAL_MAX < val)	return VAL_MAX;
		else					return val;
	}
public:
	void set_Pan(int _val)	{ int val = limit(_val); Pan = val; }
	void set_Tilt(int _val)	{ int val = limit(_val); Tilt = val; }
