...
void DMX_SYSTEM::update_Param__Test(){
	...
	LightParam[ int(DEVICE::Main) ].set_target(LIGHT_COLOR(color.r / 255.0, color.g / 255.0, color.b / 255.0, 0.0), PAN_TILT(0.0, 0.0), d_Color);
	LightParam[ int(DEVICE::Back) ].set_target(LIGHT_COLOR(color.r / 255.0, color.g / 255.0, color.b / 255.0, 0.0), PAN_TILT(0.0, 0.0), d_Color);
	
	LightParam[ int(DEVICE::Moving_L) ].set_target(LIGHT_COLOR(color.r / 255.0, color.g / 255.0, color.b / 255.0, 0.0), PAN_TILT(Gui_Global->DMX_test__Pan_L, Gui_Global->DMX_test__Tilt_L), d_Color);
	LightParam[ int(DEVICE::Moving_R) ].set_target(LIGHT_COLOR(color.r / 255.0, color.g / 255.0, color.b / 255.0, 0.0), PAN_TILT(Gui_Global->DMX_test__Pan_R, Gui_Global->DMX_test__Tilt_R), d_Color);
}
...
void DMX_SYSTEM::update_Param__InitialImmediate(){
	...
	ofColor color(0, 0, 0);
	LightParam[ int(DEVICE::Main) ].set_target(col_Black, PAN_TILT(0.0, 0.0), d_Color);
	LightParam[ int(DEVICE::Back) ].set_target(col_Black, PAN_TILT(0.0, 0.0), d_Color);
	
	LightParam[ int(DEVICE::Moving_L) ].set_target(col_Black, PAN_TILT(Gui_Global->DMX_L__Pan_init, Gui_Global->DMX_L__Tilt_init), d_Color);
	LightParam[ int(DEVICE::Moving_R) ].set_target(col_Black, PAN_TILT(Gui_Global->DMX_R__Pan_init, Gui_Global->DMX_R__Tilt_init), d_Color);
}
/******************************
******************************/
...
void DMX_SYSTEM::update_Param__Kime(){
	...
	LightParam[ int(DEVICE::Main) ].set_target(col_StrongWhite, PAN_TILT(0.0, 0.0), d_Color);
	LightParam[ int(DEVICE::Back) ].set_target(col_StrongWhite, PAN_TILT(0.0, 0.0), d_Color);
	
	LightParam[ int(DEVICE::Moving_L) ].set_target(col_StrongWhite, PAN_TILT(Gui_Global->DMX_L__Pan_Kime, Gui_Global->DMX_L__Tilt_Kime), d_Color);
	LightParam[ int(DEVICE::Moving_R) ].set_target(col_StrongWhite, PAN_TILT(Gui_Global->DMX_R__Pan_Kime, Gui_Global->DMX_R__Tilt_Kime), d_Color);
}
...
void DMX_SYSTEM::update_Param__waitKime(){
	...
	LightParam[ int(DEVICE::Moving_L) ].set_target(col_Black, PAN_TILT(Gui_Global->DMX_L__Pan_Kime, Gui_Global->DMX_L__Tilt_Kime), d_Color);
	LightParam[ int(DEVICE::Moving_R) ].set_target(col_Black, PAN_TILT(Gui_Global->DMX_R__Pan_Kime, Gui_Global->DMX_R__Tilt_Kime), d_Color);
}
...
void DMX_SYSTEM::update_Param__Figure8(int now){
	...
	LightParam[ int(DEVICE::Main) ].set_target(col_Black, PAN_TILT(0.0, 0.0), d_Color);
	LightParam[ int(DEVICE::Back) ].set_target(col_White, PAN_TILT(0.0, 0.0), d_Color);
...
void DMX_SYSTEM::update_Param__Coloring(int now){
	...
	********************/
	...
	LightParam[ int(DEVICE::Main) ].set_target(col_Performance[Order_ColPerformance[0]], PAN_TILT(0.0, 0.0), d_Color);
	LightParam[ int(DEVICE::Back) ].set_target(col_Performance[Order_ColPerformance[1]], PAN_TILT(0.0, 0.0), d_Color);
	
	LightParam[ int(DEVICE::Moving_L) ].set_target(col_Performance[Order_ColPerformance[2]], PAN_TILT(Gui_Global->DMX_L__Pan_Kime, Gui_Global->DMX_L__Tilt_Kime), d_Color);
	LightParam[ int(DEVICE::Moving_R) ].set_target(col_Performance[Order_ColPerformance[3]], PAN_TILT(Gui_Global->DMX_R__Pan_Kime, Gui_Global->DMX_R__Tilt_Kime), d_Color);	
}
...
void DMX_SYSTEM::update_DmxDataArray(){
	for(int i = 0; i < int(DEVICE::NUM_DEVICES); i++){
		...
		if(LightParam[i].get_DeviceType() == LIGHT_PARAM::DEVICE_TYPE::Fixed){
			...
			DmxData[ofs + 10]	= 0;													// Pan/ Tilt 動作速度 : 速 = 0 - 255 = 遅
			DmxData[ofs + 11]	= 0;
